...
import { getCategories, deleteOneCategory } from "../../api/categories";
...
export default function TableOrdersEntry() {
  ...
  const onDelete = async (id) => {
    if (window.confirm("¿Quieres eliminar este item?")) {
      const response = await deleteOneCategory(auth, id);
      if (response.error) {
  ...
  return (
    <div className="col-10 offset-2 table_boostrap">
              ...
              <tbody>
                {map(iterTable, (item) => (
                  <tr key={item.id}>
                    ...
                    <th>{item.attributes.nombre}</th>
                    <th>{item.attributes.descripcion}</th>
                    <td>
      ...
      {viewItem && <ViewItem viewItem={viewItem} setViewItem={setViewItem} />}
    </div>
  );
}
