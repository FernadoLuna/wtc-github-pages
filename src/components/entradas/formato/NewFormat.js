...
import {
  newOneFormatEntry,
  updateOneFormatEntry,
} from "../../../api/formats_entry";
...
export default function NewFormat(props) {
  ...
  const onSubmit = async (data) => {
    ...
    const response = await newOneFormatEntry(auth, data);
    if (response.error) {
    ...
    if (response.data.id) {
      const getFolio = `E-${dateConvert()}-${response.data.id}`;

      const setFolio = await updateOneFormatEntry(auth, {
        id: response.data.id,
        arguments: {
          folio: getFolio,
        },
  ...
  return (
    <div className="modal_info_client_large col-12 px-5 py-5">
      <form onSubmit={handleSubmit(onSubmit)}>
        <div className="row">
          ...
          <div className="col-6 mb-2">
            <label className="form-label">Pedimento WTC</label>
            <input
              type="text"
              className="form-control"
          ...
          <div className="col-12 mb-2">
            ...
            <input
              type="text"
              className="form-control"
              {...register("ubicacion_inicial", { required: true })}
            />
          </div>
