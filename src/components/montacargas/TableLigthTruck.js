...
import EditLigthTruck from "./EditLigthTruck";
import ViewLigthTruck from "./ViewLigthTruck";
import NewLigthTruck from "./NewLigthTruck";
import { getLigthTrucks, deleteOneLigthTruck } from "../../api/lighttrucks";
import { FaEye, FaMarker, FaTrashAlt } from "react-icons/fa";

export default function TableLigthTrucks() {
  const { auth } = useAuth();
  ...
  useEffect(() => {
    (async () => {
      const response = await getLigthTrucks(auth);
      if (response.data) {
  ...
  const onDelete = async (id) => {
    if (window.confirm("¿Quieres eliminar este item?")) {
      const response = await deleteOneLigthTruck(auth, id);
      if (response.error) {
  ...
  return (
    <div className="col-10 offset-2 table_boostrap px-5">
              ...
              <tbody>
                {map(iterTable, (item) => (
                  <tr key={item.id}>
                    ...
                    <th scope="row">{item.attributes.epc}</th>
                    <td>
      ...
      {viewItem && (
        <ViewLigthTruck viewItem={viewItem} setViewItem={setViewItem} />
      )}
