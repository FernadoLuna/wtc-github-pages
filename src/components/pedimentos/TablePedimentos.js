...
import { deleteOnePediment, getPediments } from "../../api/pediments";
...
export default function TablePortals() {
  ...
  useEffect(() => {
    (async () => {
      const response = await getPediments(auth);
  ...
  const onDelete = async (id) => {
    if (window.confirm("¿Quieres eliminar este item?")) {
      const response = await deleteOnePediment(auth, id);
      if (response.error) {
              ...
              <tbody>
                {map(iterTable, (item) => (
                  <tr key={item.id}>
                    ...
                    <th scope="row">
                      <a href={`${API_URL}${item.url}`} target="_blank">
