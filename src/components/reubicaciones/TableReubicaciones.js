...
import EditReubicate from "./EditReubication";

import { getReubications, deleteOneReubication } from "../../api/reubications";
import { FaEye, FaMarker, FaTrashAlt } from "react-icons/fa";

export default function TableReubicaciones() {
  const { auth } = useAuth();
  ...
  useEffect(() => {
    (async () => {
      const response = await getReubications(auth);
      if (response.data) {
  ...
  const onDelete = async (id) => {
    if (window.confirm("¿Quieres eliminar este item?")) {
      const response = await deleteOneReubication(auth, id);
      if (response.error) {
          ...
          <div className="row">
            <table className="table table-striped">
              <thead>
                <tr>
                  <th scope="col">ID</th>
                  <th scope="col">Folio de formato entrada</th>
                  <th scope="col">Fecha de movimiento</th>
                  <th scope="col">Ubicacion nueva</th>
              ...
              <tbody>
                {map(iterTable, (item) => (
                  <tr key={item.id}>
                    ...
                    <th scope="row">
                      {item.attributes.formato_entrada.data
                        ? item.attributes.formato_entrada.data.attributes.folio
                        : "s/n"}
                    </th>
                    <th scope="row">{formatDate(item.attributes.updatedAt)}</th>
                    <th scope="row">{item.attributes.nueva_ubicacion}</th>
                    <th scope="row">
                    ...
                    <td>
                      <ul className="list-icons">
                        ...
                        <li
                          data-toggle="modal"
                          data-target="#modalEditReubicate"
