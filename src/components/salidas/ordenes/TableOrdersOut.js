...
import { getOrdersOut, deleteOneOrderOut } from "../../../api/orders_out";
import NewOrderOut from "./NewOrderOut";
import EditOrderOut from "./EditOrderOut";
import ShowOrderOut from "./ShowOrderOut";
...
export default function TableOrdersEntry() {
  ...
  useEffect(() => {
    (async () => {
      const response = await getOrdersOut(auth);
      if (response.data) {
  ...
  const onDelete = async (id) => {
    if (window.confirm("¿Quieres eliminar este item?")) {
      const response = await deleteOneOrderOut(auth, id);
      if (response.error) {
  ...
  return (
    <div className="col-10 offset-2 table_boostrap">
      ...
      {viewItem && (
        <ShowOrderOut viewItem={viewItem} setViewItem={setViewItem} />
      )}
    </div>
  );
}
