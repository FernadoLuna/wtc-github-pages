...
import { getStores, deleteOneStore } from "../../api/stores";
...
export default function TablePortals() {
  ...
  useEffect(() => {
    (async () => {
      const response = await getStores(auth);
      if (response.data) {
  ...
  const onDelete = async (id) => {
    if (window.confirm("¿Quieres eliminar este item?")) {
      const response = await deleteOneStore(auth, id);
      if (response.error) {
  ...
  return (
    <div className="col-10 offset-2 table_boostrap px-5">
              ...
              <tbody>
                {map(iterTable, (item) => (
                  <tr key={item.id}>
                    ...
                    <th scope="row">{item.attributes.token}</th>
      ...
      {viewItem && <ViewStore viewItem={viewItem} setViewItem={setViewItem} />}
    </div>
  );
}
